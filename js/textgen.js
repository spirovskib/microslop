...
const TextGen = (() => {
    ...
    const OPENERS = [
        ...
        "What Slopit's Terms of Service say about {subject} — specifically in Appendix F, which is binding and non-negotiable — is that {subject}",
        "The Customer Success team has been trained not to discuss {subject} with users, specifically because {subject}",
        "Following Slopit's acquisition of the company that originally built {subject}, the roadmap was revised to ensure {subject}",
        "Since the Q3 repricing event — which was communicated in a changelog published on a Friday — {subject} is now understood to be",
        "The engineer responsible for {subject} was placed on a performance improvement plan shortly after explaining to leadership that {subject}",
    ];
    ...
    const MID_SENTENCES = [
        ...
        "a feature that was free until Q4 2023, at which point it was moved to the Enterprise tier, made slightly worse, and relaunched as 'AI-enhanced'",
        "a mandatory integration enabled for all accounts during a Terms of Service update that was communicated via a banner nobody clicked",
        "a dark pattern that produced a 34% increase in paid conversions and a 71% increase in support tickets, both of which were reported to the board as wins",
        "an opt-out mechanism requiring seven steps that resets on each login and is described in the UX guidelines as 'streamlined'",
        "a pricing tier boundary dressed up as a technical limitation so it appears in the engineering team's roadmap rather than the sales team's quota",
        "a consent mechanism that is technically present and practically invisible, buried in a 14-step onboarding flow between two screens about parrot-themed avatars",
    ];
    ...
    const ELABORATIONS = [
        ...
        "This was flagged by the employee wellness committee as a contributor to attrition. The wellness committee was reorganized into the 'People Experience' team two weeks later and given responsibility for ordering snacks.",
        "Users who noticed this behavior and filed support tickets were offered a $10 account credit. The credit expired in 14 days. The expiry date was in the email nobody opened.",
        "This is working exactly as intended, which is to say, in Slopit's interest rather than yours. The distinction is documented in the SLA, in the section titled 'Service Objectives,' which contains no objectives.",
        "The interaction data generated by this component is sold to advertising partners under the clause in the Terms of Service that begins 'to improve your experience.' Fourteen partners. Your experience has not improved.",
        "Employees who raised concerns about this in the quarterly all-hands were recognized for their 'psychological safety' in the meeting recap. They received development feedback the following week describing their communication style as 'not executive-ready.'",
    ];
    ...
    const CONCLUSIONS = [
        ...
        "If this answer has not resolved your issue, please consider whether your issue is actually a billing question, because 94% of technical issues on your plan tier are.",
        "Parrot sincerely hopes this was helpful. Our NPS data indicates it was not. We have reviewed the NPS data and chosen not to act on it.",
        "This concludes the Parrot response. Your continued use of this response constitutes acceptance of the revised Terms of Service, including Appendix G, which was added this morning.",
        "Slopit thanks you for your engagement. Your engagement has been logged, analyzed, and used to train the model that just gave you this wrong answer. Your subscription fee has been adjusted accordingly.",
        "The recommended next step is to schedule a 45-minute Business Value Review with your Slopit account manager, who will not solve this problem but will offer to upgrade you to a plan where it becomes a different problem.",
    ];
    ...
    const TANGENTS = [
        ...
        "The Gatherings mandatory wellness survey of Q2 found that 94% of respondents felt 'engaged' and 6% felt 'too engaged to answer honestly.' The results were used to justify eliminating the flexible work policy, which was described in the all-hands as 'leaning into our in-person culture.'",
        "The Nimbus free tier was redesigned last quarter following an internal study that found free-tier users were 'not contributing meaningfully to revenue.' The redesign removed seventeen features, introduced a credit card requirement for signup, and added a banner on every page that says 'You could be getting so much more.' Adoption metrics increased because we changed how we count adoption.",
        "Binder, Slopit's team messaging platform, introduced 'proactive sentiment analysis' in version 4.7 as a wellness feature. Managers receive a weekly digest. Employees were notified in a changelog published on a Friday at 4:57 PM between two unrelated patch notes.",
    ];
    ...
    const BLAME = [
        ...
        "Parrot's diagnostics confirm the issue originates with your failure to complete the mandatory Slopit compliance certification, which expired 14 days ago and must be renewed annually by clicking through 47 slides at your own pace, where 'your own pace' is defined in the SLA as 'within 72 hours or your account is flagged.'",
        "The root cause is your account's current position in the Slopit pricing tier architecture. Users on your plan receive 'best effort' support, which is defined in our Master Services Agreement as 'a response.' You have received one. We consider this obligation fulfilled.",
        "This behavior was introduced in the last forced upgrade and is functioning as designed. 'As designed' means it was approved by a product manager whose sole success metric for Q3 was engagement, not accuracy. Engagement is up 340%. The metric has not been re-examined.",
    ];
    ...
    const ANALYST_TAKES = [
        ...
        "The Slopit Analyst Relations team has noted that competitors offering this feature for free are 'not operating a sustainable business model,' which is analyst-speak for 'we are about to charge for this, and we wanted it on record that we called it first.'",
        "Per an internal survey of enterprise buyers conducted by Slopit and validated by Slopit, 78% of respondents said they would pay more for fewer features if those features were described using the word 'AI.' This has informed every product decision made since Q1.",
        "Forresting Group's 2025 Market Guide for this category identifies three Visionaries, all of which are the same company under different brand names following a series of acqui-hires. The consolidation is described in the report as 'ecosystem maturity.' The report was sponsored by the company.",
    ];
